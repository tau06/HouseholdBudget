...
import jp.ne.nissing.util.householdbudget.data.*;
...
import jp.ne.nissing.util.householdbudget.forms.*;
...
public class HouseholdBudget extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tab01.setIndicator(getText( R.string.expenditure), getResources().getDrawable(R.drawable.ic_tab_expenditure));
		tab01.setContent(R.id.expenditure_tab);
		tabs.addTab(tab01);

		TabSpec tab02 = tabs.newTabSpec(getText(R.string.tab_id_income).toString());
		tab02.setIndicator(getText(R.string.income), getResources().getDrawable(R.drawable.ic_tab_income));
		tab02.setContent(R.id.income_tab);
		tabs.addTab(tab02);

		TabSpec tab03 = tabs.newTabSpec(getText(R.string.tab_id_move).toString());
		tab03.setIndicator(getText(R.string.move), getResources().getDrawable(R.drawable.ic_tab_move));
				...
				if(category.getID() == DIRECT_INPUT_CATEGORY_ID){
					...
					.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							String instantCategoryStr = editView.getText().toString();
							Category instantCategory = new Category(INSTANCE_CATEGORY, instantCategoryStr,
									HouseholdData.getInstance().getPaymentMethod().getMethodByName("現金").getId(), false);
							...
							for(int i = 0 ; i < categorySpinner.getCount(); i++){
								Category target = (Category) categorySpinner.getItemAtPosition(i);
					...
					editView.setOnFocusChangeListener(new OnFocusChangeListener() {
						...
						public void onFocusChange(View v, boolean hasFocus) {
							if(hasFocus)
								alartDialog.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_VISIBLE);
						}
					});
					alartDialog.show();
	...
	protected void onStart() {
		...
		// 支払方法スピナー設定
		ArrayAdapter<PaymentMethod> expenditureAdapter = new ArrayAdapter<PaymentMethod>(this, android.R.layout.simple_spinner_item);
		...
		//入金先スピナー設定
		ArrayAdapter<CashStorage> recieveAdapter = new ArrayAdapter<CashStorage>(this, android.R.layout.simple_spinner_item);
		...
		//移動元,移動先スピナー設定
		ArrayAdapter<CashStorage> movePaymentAdapter = new ArrayAdapter<CashStorage>(this, android.R.layout.simple_spinner_item);
	...
	private ArrayAdapter<Category> createCategoryArrayAdapter() {
		ArrayAdapter<Category> categoryAdaper = new ArrayAdapter<Category>(this, android.R.layout.simple_spinner_item);
		...
		Category instanceCategory = new Category(DIRECT_INPUT_CATEGORY_ID, "直接入力...",
				HouseholdData.getInstance().getPaymentMethod().getMethodByName("現金").getId(), false);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		menu.add(Menu.NONE, MENU_ID_MENU1, Menu.NONE, getText(R.string.setting)).setIcon(android.R.drawable.ic_menu_manage);
	...
	private Handler sendLocalToastHandler = new Handler() {
		public void handleMessage(Message msg) {
			Toast.makeText(getApplicationContext(), getText(R.string.send_local), Toast.LENGTH_SHORT).show();
		}
	};
	...
	private void sendExpenditureData() {
		...
		String cashStorage = HouseholdData.getInstance().getPaymentMethod()
				.getMethodByName(howToPay).getPaymentCashStorage()
				.getCashStorageName();

		RowElement rowElement = RowElementFactory.createPayRowElement(
				parseButtonText(expenditureDateButton), Integer.parseInt(money), cashStorage, category, howToPay, memo);
	...
	private void sendMoveData() {
		...
		RowElement moveFromRowElement = RowElementFactory
				.createMoveFromRowElement(parseButtonText(moveDateButton), Integer.parseInt(money), moveFrom, memo);
		RowElement moveToRowElement = RowElementFactory
				.createMoveToRowElement(parseButtonText(moveDateButton), Integer.parseInt(money), moveTo, memo);
