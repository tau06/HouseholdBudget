...
import jp.ne.nissing.util.householdbudget.data.cashstorage.*;
import jp.ne.nissing.util.householdbudget.data.category.*;
import jp.ne.nissing.util.householdbudget.data.paymentmethod.*;
...
public class DBManager {
	...
	public void buildHouseholdData(){
		...
		HouseholdData.getInstance().setProfile(profile.getAccountName(), profile.getPassword(), profile.getSheetName());
	...
	/**
	 * アカウント情報を書き込む
	 * @param accountName アカウント名（メールアドレス）
	 * @param password パスワード
	 * @param spreadSheetName スプレッドシート名
	 */
	public void writeProfile(String accountName, String password, String spreadSheetName){
		String sql = "INSERT INTO profile VALUES('"  + accountName + "','" + password + "','" + spreadSheetName + "');";
	...
	public void addCategory(String category, int paymethodID){
		...
		query = "INSERT INTO category VALUES('" + id + "','" + category + "','" + paymethodID + "','false');";
	...
	public void updateCategory(String category, String paymentMethod){
		String sql = "UPDATE category SET defaultpay = '" + paymentMethod + "' WHERE name = '" + category + "'";
		database.execSQL(sql);
	}
	 ...
	 */
	public void replaceCategoryPaymentMethodRelation(String beforePaymentMethod, String afterPaymentMethod) {
		...
		sql = "UPDATE category SET paymethod_id = '" + afterPaymentId + "' WHERE paymethod_id = '" + beforePaymentId + "'";
		database.execSQL(sql);
	...
	public void addPaymentMethod(String method, String cashStorage){
		...
		String sql = "INSERT INTO paymethod VALUES('" + id + "', '" + method + "','" + cashStorage + "','false');";
		database.execSQL(sql);
		database.close();
	}
	 ...
	 * 支払方法を更新する
	 * @param method 支払方法
	 * @param cashStorage 現金保管場所
	 */
	public void updatePaymentMethod(String method, String cashStorage){
		String sql = "UPDATE payment SET cash_storage = '" + cashStorage + "' WHERE name ='" + method + "'";
		database.execSQL(sql);
	}
	 ...
	 */
	public void replacePaymentMethodCashStorageRelation(String beforeCashStorage, String afterCashStorage) {
		...
		sql = "UPDATE paymethod SET cash_storage_id = '" + afterCashStorageId + "' WHERE cash_storage_id = '" + beforeCashStorageId + "'";
		try{
		database.execSQL(sql);
		}catch(Exception e){
			String s = e.toString();
	...
	public void addCashStorage(String cashStorage){
		...
		String sql = "INSERT INTO cash_storage VALUES('" + id + "', '" + cashStorage + "' ,'false');";
		database.execSQL(sql);
		database.close();
	}
