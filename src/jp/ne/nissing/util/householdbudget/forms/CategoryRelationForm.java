...
public class CategoryRelationForm extends Activity implements OnClickListener{
	...
	public void onCreate(Bundle savedInstanceState){
		...
		ArrayAdapter<String> attributeAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_spinner_item);
	...
	public void onClick(View v) {
		...
		// 未入力個所があればキャンセル
		if(category.getSelectedItem().toString().equals("") || payMethod.getSelectedItem().toString().equals("")){
			showToastMessage("テキストエリアに入力してください");
			return;
		}
		
		DBManager.getInstance().updateCategoryPayMethodRelation(category.getSelectedItem().toString() , payMethod.getSelectedItem().toString());
		
		showToastMessage(category.getSelectedItem().toString() + "のデフォルトを" + payMethod.getSelectedItem().toString() + "に設定しました");
